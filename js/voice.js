...
window.__Voice = {
    ...
    voiceAnalysis : function (audioData) {
        ...
        const convertIntoBlobfile = function(audioData){
            let dataview = encodeWAV(mergeBuffers(audioData), __Voice.audio_sample_rate); //11025以外の数字に設定すると"invalid sample-rate. is(22050.000000), expected(11025.000000)."というメッセージがAPIから帰ってくる
...
const onAudioProcess = function (e) {
    ...
    var bufferData = new Float32Array(__Voice.bufferSize);
    for (var i = 0; i < __Voice.bufferSize; i++) {
...
const recorder = function (){
    ...
    setTimeout(function () {
        ...
        __Voice.voiceAnalysis(audioData);//APIに送る
        audioContext.close()
...
const handleSuccess = function (stream) {
    ...
    scriptProcessor = audioContext.createScriptProcessor(__Voice.bufferSize, 1, 1);
...
//5秒に1回録音する
const executeAnalysis = async function() {
...
// startvoiceBtn.addEventListener("click",executeAnalysis)
